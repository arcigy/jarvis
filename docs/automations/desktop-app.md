...
- After wake, Jarvis can answer voice commands for cold outreach, integrations health, production completion percent, email identity, lead discovery, and Gemini reply drafts; cold outreach prefers live Smartlead statistics and falls back to local SQLite.
- The first viewport uses a committed PNG command-core visual generated by `scripts/generate_desktop_visual_asset.py`.
- `Operator brief` creates one Jarvis answer from readiness, cold outreach, client requests, and prepared reply approvals; explicit live runs sync recent Gmail messages before reading client requests.
- `Jarvis full launch proof` reads one combined readiness, production evidence, and remote MCP handoff summary without running write actions.
- Gmail sync defaults to `in:inbox newer_than:7d`; live briefing uses `in:inbox newer_than:2d`.
- The operator briefing loads automatically on startup, refreshes every five minutes, and speaks only when the user clicks `Operator brief`.
...
- The client memory panel starts a local watch loop that refreshes open requests every minute, speaks only newly detected client needs, and raises a desktop notification for new client requests when notifications are allowed.
