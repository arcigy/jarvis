...
- `arcigy.build_pricing_inventory_guard_preview`: read-only pricing MCP guard for product/service availability, discount cap, minimum total, and margin before preparing an offer.
...
- `arcigy.build_gmail_outreach_readiness_preview`: read-only Gmail cold outreach readiness runbook for account auth, `COLD-OUTREACH` label state, unread lead replies, known-lead matching, draft-only next steps, and approval-gated thread label payloads.
...
- `arcigy.build_international_market_leadgen_preview`: plans AU/UK/US-style market leadgen with Places/Serper discovery, import, scrape, AI intro work packets, and Smartlead package next steps.
...
- `arcigy.build_smartlead_fixed_campaign_package_preview`: recreates the old fixed Smartlead injector flow as a read-only package with fixed sequence copy, stop-on-reply settings, webhook, schedule, QA, and approval payloads.
...
- `arcigy.build_leadgen_progress_watchdog_preview`: builds a read-only percent-complete watchdog for leadgen runs, with grouped bottlenecks and scrape/fetch/AI/QA/Smartlead next calls.
- `arcigy.build_leadgen_target_backfill_preview`: builds a read-only target backfill runbook from old check-progress/check-ORSR/check-icebreaker flows, splitting leads into missing email/scrape, decision maker/ORSR, AI intro, verification retry, and Smartlead audit queues.
- `arcigy.build_leadgen_maintenance_runbook_preview`: builds a read-only maintenance runbook from DB status, CSV/leads, Smartlead campaign drift, and Gmail label readiness, covering missing emails, AI intros, company short names, ICO, sync, and label setup next steps.
...
- `arcigy.build_maps_cold_calling_export_preview`: prepares a deduplicated cold-calling CSV from Google Maps/CSV leads with phones, separates missing-phone scrape work, and keeps export approval-gated.
