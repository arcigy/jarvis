...
async function verifyRemoteMcpSmoke(publicUrl: string, token: string | null): Promise<RemoteMcpSmoke | null> {
  ...
  const requiredChecks = ["action-manifest", "openapi-schema", "cors-preflight", "external-auth-gate", "pack-limits", "approval-gate", "approval-shape-gate", "secret-redaction"];
...
function renderTunnelReadySummary(input: {
  ...
  return [
    ...
    "- Required proof before work: action manifest HTTP 200, OpenAPI schema HTTP 200, manifest HTTP 200, connection pack tokenValueReturned=false with repo-only limits, remote smoke status=ready with action-manifest, openapi-schema, cors-preflight, external-auth-gate, pack-limits, approval-gate, approval-shape-gate, and secret-redaction.",
