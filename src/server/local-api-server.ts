...
async function routeMcpTool(name: string, request: IncomingMessage, response: ServerResponse) {
  ...
  if (name === "arcigy.get_gmail_lead_context") {
    writeJson(response, 200, {
      result: await fetchGmailLeadContext({
        leadEmail: String(payload.leadEmail ?? ""),
        accountEnvKey: optionalString(payload.accountEnvKey),
  ...
  if (name === "arcigy.build_smartlead_safe_sync_runbook_preview") {
    writeJson(response, 200, {
      result: buildSmartleadSafeSyncRunbookPreview({
        campaignId: typeof payload.campaignId === "string" || typeof payload.campaignId === "number" || payload.campaignId === null ? payload.campaignId : undefined,
        campaignName: optionalString(payload.campaignName),
        localLeads: (payload.localLeads ?? []) as Parameters<typeof buildSmartleadSafeSyncRunbookPreview>[0]["localLeads"],
        remoteLeads: Array.isArray(payload.remoteLeads) ? payload.remoteLeads as Parameters<typeof buildSmartleadSafeSyncRunbookPreview>[0]["remoteLeads"] : undefined,
        campaignSnapshot: isRecord(payload.campaignSnapshot) ? payload.campaignSnapshot as Parameters<typeof buildSmartleadSafeSyncRunbookPreview>[0]["campaignSnapshot"] : undefined,
        updateExisting: payload.updateExisting !== false,
        requirePause: payload.requirePause !== false,
        includeBackupPlan: payload.includeBackupPlan !== false,
      }),
    });
    return;
  }
  ...
  if (name === "arcigy.build_smartlead_campaign_delete_safety_preview") {
    writeJson(response, 200, {
      result: buildSmartleadCampaignDeleteSafetyPreview({
        campaigns: Array.isArray(payload.campaigns) ? payload.campaigns as Parameters<typeof buildSmartleadCampaignDeleteSafetyPreview>[0]["campaigns"] : undefined,
        backupPlan: payload.backupPlan as Parameters<typeof buildSmartleadCampaignDeleteSafetyPreview>[0]["backupPlan"],
        backupRunId: optionalString(payload.backupRunId),
        backedUpCampaignIds: Array.isArray(payload.backedUpCampaignIds) ? payload.backedUpCampaignIds as Array<string | number> : undefined,
        protectedCampaignIds: Array.isArray(payload.protectedCampaignIds) ? payload.protectedCampaignIds as Array<string | number> : undefined,
        protectedNameParts: Array.isArray(payload.protectedNameParts) ? payload.protectedNameParts.map(String) : undefined,
        backupRoot: optionalString(payload.backupRoot),
        requireFullBackupEvidence: payload.requireFullBackupEvidence !== false,
        maxDeleteCandidates: typeof payload.maxDeleteCandidates === "number" ? payload.maxDeleteCandidates : undefined,
        operatorPhrase: optionalString(payload.operatorPhrase),
      }),
    });
    return;
  }
  ...
  if (name === "arcigy.build_leadgen_db_status_preview") {
    writeJson(response, 200, {
      result: buildLeadgenDbStatusPreview({
        leads: Array.isArray(payload.leads) ? payload.leads as Parameters<typeof buildLeadgenDbStatusPreview>[0]["leads"] : undefined,
        csvText: optionalString(payload.csvText),
        delimiter: payload.delimiter === ";" ? ";" : payload.delimiter === "," ? "," : undefined,
        sourceName: optionalString(payload.sourceName),
        niches: Array.isArray(payload.niches) ? payload.niches as Parameters<typeof buildLeadgenDbStatusPreview>[0]["niches"] : undefined,
        resumeStates: Array.isArray(payload.resumeStates) ? payload.resumeStates as Parameters<typeof buildLeadgenDbStatusPreview>[0]["resumeStates"] : undefined,
        blacklistDomains: Array.isArray(payload.blacklistDomains) ? payload.blacklistDomains.map(String) : undefined,
  ...
  if (name === "arcigy.build_phone_enrichment_queue_preview") {
    writeJson(response, 200, {
      result: buildPhoneEnrichmentQueuePreview({
        leads: Array.isArray(payload.leads) ? payload.leads as Parameters<typeof buildPhoneEnrichmentQueuePreview>[0]["leads"] : undefined,
        csvText: optionalString(payload.csvText),
        delimiter: payload.delimiter === ";" ? ";" : payload.delimiter === "," ? "," : undefined,
        sourceName: optionalString(payload.sourceName),
  ...
  if (name === "arcigy.build_ai_intro_work_packet_preview") {
    writeJson(response, 200, {
      result: buildAiIntroWorkPacketPreview({
        leads: Array.isArray(payload.leads) ? payload.leads as Parameters<typeof buildAiIntroWorkPacketPreview>[0]["leads"] : undefined,
        csvText: optionalString(payload.csvText),
        delimiter: payload.delimiter === ";" ? ";" : payload.delimiter === "," ? "," : undefined,
        sourceName: optionalString(payload.sourceName),
        niche: optionalString(payload.niche),
  ...
  if (name === "arcigy.build_ai_intro_import_preview") {
    writeJson(response, 200, {
      result: buildAiIntroImportPreview({
        leads: Array.isArray(payload.leads) ? payload.leads as Parameters<typeof buildAiIntroImportPreview>[0]["leads"] : undefined,
        csvText: optionalString(payload.csvText),
        delimiter: payload.delimiter === ";" ? ";" : payload.delimiter === "," ? "," : undefined,
        sourceName: optionalString(payload.sourceName),
        niche: optionalString(payload.niche),
        ...
        maxLeads: typeof payload.maxLeads === "number" ? payload.maxLeads : undefined,
        maxContextChars: typeof payload.maxContextChars === "number" ? payload.maxContextChars : undefined,
        completedIntros: Array.isArray(payload.completedIntros) ? payload.completedIntros as Parameters<typeof buildAiIntroImportPreview>[0]["completedIntros"] : undefined,
        resultJsonText: optionalString(payload.resultJsonText),
        resultCsvText: optionalString(payload.resultCsvText),
        resultDelimiter: payload.resultDelimiter === ";" ? ";" : payload.resultDelimiter === "," ? "," : undefined,
      }),
    });
    return;
  }
  if (name === "arcigy.build_ai_icebreaker_writeback_preview") {
    writeJson(response, 200, {
      result: buildAiIcebreakerWritebackPreview({
        leads: Array.isArray(payload.leads) ? payload.leads as Parameters<typeof buildAiIcebreakerWritebackPreview>[0]["leads"] : [],
        icebreakers: Array.isArray(payload.icebreakers) ? payload.icebreakers as Parameters<typeof buildAiIcebreakerWritebackPreview>[0]["icebreakers"] : undefined,
        resultJsonText: optionalString(payload.resultJsonText),
        sourceName: optionalString(payload.sourceName),
        niche: optionalString(payload.niche),
        offer: optionalString(payload.offer),
