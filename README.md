...
7. Run `arcigy.run_remote_mcp_smoke` or `GET /api/remote-mcp-smoke` before handoff when you need proof that action manifest, OpenAPI schema, CORS preflight, external auth gate, auth throttle policy, Jarvis manifest, tunnel controls, exact tool registry, read-only calls, audit quick-start, production evidence quick-start, completion score quick-start, production evidence tool call, approval gates, and token redaction work. `npm run web:tunnel` refuses handoff unless that same smoke proof is ready.
8. Before any remote agent proposes work, require `arcigy.get_jarvis_capability_audit` and `arcigy.get_production_completion_score`, then cite coverage, completion percent, MCP counts, and evidence status.
