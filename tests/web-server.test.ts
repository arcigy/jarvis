...
test("local web bridge serves UI and API health", async () => {
  ...
  try {
    ...
    assert.ok(openApiBody.paths["/api/mcp/arcigy.build_leadgen_progress_watchdog_preview"]);
    assert.ok(openApiBody.paths["/api/mcp/arcigy.build_leadgen_target_backfill_preview"]);
    ...
    assert.ok(openApiBody.paths["/api/mcp/arcigy.build_leadgen_maintenance_runbook_preview"]);
    ...
    assert.ok(openApiBody.paths["/api/mcp/arcigy.build_smartlead_campaign_audit_preview"]);
    assert.ok(openApiBody.paths["/api/mcp/arcigy.build_smartlead_workspace_diagnostic_preview"]);
    assert.ok(openApiBody.paths["/api/mcp/arcigy.build_smartlead_message_history_audit_preview"]);
    ...
    assert.ok(openApiBody.paths["/api/mcp/arcigy.build_pricing_inventory_guard_preview"]);
    ...
    const openApiPricingInventoryGuard = openApiBody.paths["/api/mcp/arcigy.build_pricing_inventory_guard_preview"] as OpenApiPathFixture;
    ...
    assert.ok(Array.isArray(openApiPricingInventoryGuard.post.requestBody.content["application/json"].examples.quickStart.value.products));
    ...
    assert.ok(remotePackBody.quickStartCalls.some((call) => call.tool === "arcigy.build_smartlead_campaign_audit_preview" && Array.isArray(call.body.campaigns)));
    assert.ok(remotePackBody.quickStartCalls.some((call) => call.tool === "arcigy.build_smartlead_workspace_diagnostic_preview" && call.approvalRequired === false && Array.isArray(call.body.endpointChecks)));
    assert.ok(remotePackBody.quickStartCalls.some((call) => call.tool === "arcigy.build_smartlead_message_history_audit_preview" && call.approvalRequired === false && Array.isArray(call.body.leads)));
    ...
    assert.ok(remotePackBody.quickStartCalls.some((call) => call.tool === "arcigy.build_leadgen_progress_watchdog_preview" && call.body.targetReadyLeads === 50));
    assert.ok(
      remotePackBody.quickStartCalls.some(
        (call) =>
    ...
    assert.ok(remotePackBody.quickStartCalls.some((call) => call.tool === "arcigy.build_pricing_inventory_guard_preview" && call.approvalRequired === false && Array.isArray(call.body.products)));
    ...
    assert.ok(remotePackBody.quickStartCalls.some((call) => call.tool === "arcigy.build_leadgen_target_backfill_preview" && call.approvalRequired === false && call.body.minReadyLeads === 50));
