...
test("desktop shell exposes Jarvis wake-word UI and safe preload boundary", () => {
  ...
  assert.match(renderer, /window\.confirm\(`Exportovat \$\{state\.lastLeads\.length\} leadov do Google Sheets\?`\)/);
  ...
  assert.match(renderer, /window\.confirm\(`Schvalit pripravenu odpoved pre \$\{first\.leadEmail\}/);
  assert.match(renderer, /window\.confirm\(`Odoslat schvalenu odpoved pre \$\{first\.leadEmail\}/);
